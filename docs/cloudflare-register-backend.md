...
- 报名成功返回 UUID 与**登录链接** `https://<域名>/register?uuid=<uuid>`（任何设备打开即自动登录）
- 登录态存于 localStorage（`mc-event:session`）；带 `?uuid=` 打开时会自动写入会话并从地址栏抹除
- 登录后登记处为**查看模式**（只读），点「启用修改」才能编辑；学号永久锁定（变更需联系负责人）
- 同一电脑换人：退出登录或以新 UUID 登录时，自动清空前一位的草稿与本机记录，防止表单串号
